...
class MazeGenerator:
    ...
    def __init__(self, width: int, height: int, seed: int, p_flag: bool) ->  None:
    ...
    def generate(self, entry: tuple[int, int], end: tuple[int, int]) -> None:
        ...
        stack = [(0,0)]
        is_visited: list[list[bool]] = [[False] * self.width for _ in range(self.height)]
        is_visited[0][0] = True
        while stack:
            sides = []
            x, y = stack[-1]
            if x > 0 and not is_visited[y][x-1] and (x-1, y) not in logo:
                sides.append((x-1, y))
            if x < self.width -1 and not is_visited[y][x+1] and (x+1, y) not in logo:
                sides.append((x+1, y))
            if y > 0 and not is_visited[y-1][x] and (x, y-1) not in logo:
                sides.append((x, y-1))
            if y < self.height -1 and not is_visited[y+1][x] and (x, y+1) not in logo:
                sides.append((x, y+1))
            if sides:
    ...
    def get_solution(self, entry: tuple[int, int], end: tuple[int, int]) -> None:
        ...
        Returns:
            None
        """
        came_from: list[list[tuple[int, int] | None]] = [[None] * self.width for _ in range(self.height)]
        ...
        while queue:
            ...
            if (cx, cy) == end:
                break
            if cy > 0 and self.grid[cy][cx] & 0x1 == 0 and came_from[cy-1][cx] is None:
                queue.append((cx, cy-1))
                came_from[cy-1][cx] = (cx, cy)
            if cx < self.width -1 and self.grid[cy][cx] & 0x2 == 0 and came_from[cy][cx+1] is None:
                queue.append((cx+1, cy))
                came_from[cy][cx+1] = (cx, cy)
            if cy < self.height -1 and self.grid[cy][cx] & 0x4 == 0 and came_from[cy+1][cx] is None:
                queue.append((cx, cy+1))
                came_from[cy+1][cx] = (cx, cy)
            if cx > 0 and self.grid[cy][cx] & 0x8 == 0 and came_from[cy][cx-1] is None:
                queue.append((cx-1, cy))
                came_from[cy][cx-1] = (cx, cy)
    ...
    def draw_42(self, entry: tuple[int, int], end: tuple[int, int]) -> list:
        ...
        pattern_cells = {(x+start_x, y+start_y) for x, y in four} | {(x+start_x, y+start_y) for x, y in two}
